...
class CachedMacros[C <: Context](c0: C) extends LazyMacros(c0) {
  ...
  def materializeCached[T: WeakTypeTag]: Tree = {
    ...
    if (CachedMacros.deriving || concurrentLazy) {
      ...
      q"_root_.shapeless.compat.Cached($tree0)"
    } else {
      ...
      try {
        ...
        treeOpt.getOrElse {
          ...
          val tree = mkImpl[T](
            (tree, actualType) => q"_root_.shapeless.compat.Cached[$actualType]($tree)",
            q"null.asInstanceOf[_root_.shapeless.compat.Cached[_root_.scala.Nothing]]"
          )
